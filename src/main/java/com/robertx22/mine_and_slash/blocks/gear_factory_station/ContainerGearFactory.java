...
import com.robertx22.mine_and_slash.blocks.slots.FactorySlot;
...
public class ContainerGearFactory extends BaseTileContainer {
    ...
    public static final int FUEL_SLOTS_COUNT = TileGearFactory.FUEL_SLOTS_COUNT;
    ...
    public ContainerGearFactory(int num, PlayerInventory invPlayer, IInventory inventory,
                                BlockPos pos) {
        super(FURNACE_SLOTS_COUNT, ContainerTypeRegisters.GEAR_FACTORY, num);
        ...
        for (int x = 0; x < HOTBAR_SLOT_COUNT; x++) {
            int slotNumber = x;
            this.addSlot(new Slot(invPlayer, slotNumber, HOTBAR_XPOS + SLOT_X_SPACING * x, HOTBAR_YPOS));
        }
        ...
        for (int x = 0; x < FUEL_SLOTS_COUNT; x++) {
            ...
            addSlot(new FuelSlot(inventory, slotNumber, FUEL_SLOTS_XPOS + SLOT_X_SPACING * x, FUEL_SLOTS_YPOS));
        }
        ...
        for (int y = 0; y < INPUT_SLOTS_COUNT; y++) {
            ...
            addSlot(new FactorySlot(inventory, slotNumber, INPUT_SLOTS_XPOS, INPUT_SLOTS_YPOS + SLOT_Y_SPACING * y));
        }

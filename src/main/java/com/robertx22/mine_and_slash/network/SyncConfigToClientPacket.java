...
public class SyncConfigToClientPacket {
    ...
    public SyncConfigToClientPacket(ListStringData configData,
                                    ConfigRegister.Config configType) {
    ...
    public static void handle(final SyncConfigToClientPacket pkt,
                              Supplier<NetworkEvent.Context> ctx) {

        ctx.get().enqueueWork(() -> {
            try {
                ConfigRegister.CONFIGS.get(pkt.configType)
                        .loadFromJsons(pkt.configData.list);
            } catch (Exception e) {
                e.printStackTrace();
            }
        });

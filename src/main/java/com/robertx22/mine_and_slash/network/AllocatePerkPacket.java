...
public class AllocatePerkPacket {

    public String guid;
    SlashRegistryType registryType;

    public AllocatePerkPacket() {

    }

    public AllocatePerkPacket(BasePerk perk) {
        this.guid = perk.GUID();
        this.registryType = perk.getSlashRegistryType();
    }

    public static AllocatePerkPacket decode(PacketBuffer buf) {

        AllocatePerkPacket newpkt = new AllocatePerkPacket();
    ...
    public static void encode(AllocatePerkPacket packet, PacketBuffer tag) {
        tag.writeString(packet.guid, 50);
        tag.writeString(packet.registryType.name(), 30);
    }

    public static void handle(final AllocatePerkPacket pkt, Supplier<NetworkEvent.Context> ctx) {

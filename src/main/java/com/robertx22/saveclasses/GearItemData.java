...
public class GearItemData implements ITooltip, ISalvagable {
  ...
  public void BuildTooltip(ItemStack stack, ItemTooltipEvent event, Unit unit, UnitData data) {
    ...
    if (this.GetBaseGearType() instanceof IWeapon) {
      ...
      event.getToolTip().add("");
      event.getToolTip()
          .add(TextFormatting.GREEN + CLOC.stat("energy") + ": " + iwep.mechanic().GetEnergyCost());
    }
  ...
  private void BuildSetTooltip(ItemTooltipEvent event, Unit unit, UnitData data) {
    ...
    if (this.set != null) {
      event.getToolTip()
          .add(TextFormatting.GREEN + "[Set]: " + TextFormatting.GRAY + set.GetSet().Name());
      ...
      for (Entry<Integer, StatMod> entry : set.GetSet().AllMods().entrySet()) {
        ...
        if (unit.WornSets.containsKey(set.baseSet)
            && unit.WornSets.get(set.baseSet) >= entry.getKey()) {
        ...
        for (String str : StatModData.Load(entry.getValue(), set.GetSet().StatPercent)
          ...
          String str2 =
              color + "" + entry.getKey() + " set" + ": " + TextFormatting.DARK_GREEN + stat;
  ...
  public ItemStack getSalvageResult(float salvageBonus) {
    ...
    if (RandomUtils.roll(this.GetRarity().specialItemChance())) {
      ...
      Item item = (Item) RandomUtils.WeightedRandom(
          ListUtils.SameTierOrLess(ListUtils.CollectionToList(CurrencyItem.ITEMS), tier));

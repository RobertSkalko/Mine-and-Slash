...
public class TileInventoryFurnace extends TileEntity implements IInventory, ITickable {
	...
	public void update() {
		...
		if (canSmelt()) {
			...
			cookTime += 5; // numberOfFuelBurning;
		...
		// when the number of burning slots changes, we need to force the block to
		// re-render, otherwise the change in
		// state will not be visible. Likewise, we need to force a lighting
		// recalculation.
		// The block update (for renderer) is only required on client side, but the
		// lighting is required on both, since

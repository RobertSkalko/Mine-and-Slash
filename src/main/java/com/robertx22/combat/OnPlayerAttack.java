...
public class OnPlayerAttack {
    ...
    public void onPlayerAttack(LivingDamageEvent event) {
        ...
        if (event.getEntityLiving() instanceof EntityMob) {
            ...
            int maxDMG = stats.get(Stats.MAX_DAMAGE.name);

            int attackPower = stats.get(Stats.ATTACK_POWER.name);
            int magicPower = stats.get(Stats.MAGIC_POWER.name);
            int healPower = stats.get(Stats.HEAL_POWER.name);

            int DMG = minDMG + ran.nextInt(maxDMG - minDMG);

            int critChance = stats.get(Stats.CRIT_CHANCE.name);
            int critDamage = stats.get(Stats.CRIT_DAMAGE.name);

            int lifesteal = stats.get(Stats.LIFESTEAL.name);

            int fireDMG = stats.get(Stats.FIRE_DMG.name);
            int iceDMG = stats.get(Stats.ICE_DMG.name);
            int stormDMG = stats.get(Stats.STORM_DMG.name);
            int darkDMG = stats.get(Stats.DARK_DMG.name);
            int lightDMG = stats.get(Stats.LIGHT_DMG.name);

            int bonusFireDMG = stats.get(Stats.BONUS_FIRE_DMG.name);
            int bonusIceDMG = stats.get(Stats.BONUS_ICE_DMG.name);
            int bonusStormDMG = stats.get(Stats.BONUS_STORM_DMG.name);
            int bonusDarkDMG = stats.get(Stats.BONUS_DARK_DMG.name);
            int bonusLightDMG = stats.get(Stats.BONUS_LIGHT_DMG.name);

            int fireMULTI = stats.get(Stats.FIRE_DMG_MULTI.name);
            int iceMULTI = stats.get(Stats.ICE_DMG_MULTI.name);
            int stormMULTI = stats.get(Stats.STORM_DMG_MULTI.name);
            int darkMULTI = stats.get(Stats.DARK_DMG_MULTI.name);
            int lightMULTI = stats.get(Stats.LIGHT_DMG_MULTI.name);

            // ELEMENTAL DMG

            fireDMG += fireDMG * bonusFireDMG / 100;
            fireDMG += fireDMG * (float) (fireMULTI / 100);

            iceDMG += iceDMG * bonusIceDMG / 100;
            iceDMG += iceDMG * (float) (iceMULTI / 100);

            stormDMG += stormDMG * bonusStormDMG / 100;
            stormDMG += stormDMG * (float) (stormMULTI / 100);

            darkDMG += darkDMG * bonusDarkDMG / 100;
            darkDMG += darkDMG * (float) (darkMULTI / 100);

            lightDMG += lightDMG * bonusLightDMG / 100;
            lightDMG += lightDMG * (float) (lightMULTI / 100);

            int elementalDMG = fireDMG + stormDMG + iceDMG + darkDMG + lightDMG;

            //elementalDMG += magicPower * elementalDMG / 100;

            // attack power doesn't increase elemental dmg
            DMG = DMG + DMG * attackPower / 100;

            int combinedDMG = DMG + elementalDMG;

            // CRIT
            if (RandomUtils.roll(critChance)) {
                combinedDMG += combinedDMG * critDamage / 100;
            }
            // CRIT

            float mobHPMulti = GeneralUtils.getHpMultiplier(mob.getMaxHealth(),
                    mob.getCapability(EntityData.Data, null).getNBT().getInteger(Stats.HEALTH.name));

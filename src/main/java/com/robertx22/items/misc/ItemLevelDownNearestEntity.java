...
public class ItemLevelDownNearestEntity extends Item {

	@GameRegistry.ObjectHolder(Ref.MODID + ":level_down_nearest_entity")
	public static final Item ITEM = null;

	public ItemLevelDownNearestEntity() {

		RegisterItemUtils.RegisterItemName(this, "level_down_nearest_entity");
				...
				for (EntityLivingBase en : world.getEntitiesWithinAABB(EntityLivingBase.class, box)) {
					...
					if (en.isEntityEqual(player) == false && en instanceof EntityPlayer == false) {
						...
						if (data.getLevel() + 1 <= ModConfig.Server.MAXIMUM_PLAYER_LEVEL) {
							...
							player.sendMessage(SLOC.chat("you_leveleddown_entity"));
	...
	public static void registerItems(RegistryEvent.Register<Item> event) {
		event.getRegistry().register(new ItemLevelDownNearestEntity());
	}

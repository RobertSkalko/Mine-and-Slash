 ...
 *
 * This class is used to create the client and server gui elements when a player
 * opens a gui. There can only be one gui handler registered per mod, so each
 * GUI is given a unique GuiID which is used to create the correct element. The
 * GuiHandlerRegistry is registered with Forge using
 * NetworkRegistry.INSTANCE.registerGuiHandler(ReferenceMod.instance, new
 * GuiHandlerRegistry()); Each of the examples, with its own GuiHandler,
 * registers itself with the GuiHandlerRegistry. When GuiHandlerRegistry
 * receives a call from Forge, it passes it to the appropriate example's
 * GuiHandler.
 */
